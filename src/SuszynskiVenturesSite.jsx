...
/>
      ...
      <section className="relative z-10 flex flex-col items-center justify-center px-4 py-24 text-center">
        <motion.div style={{ scale }} className="max-w-3xl">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <a href="#contact" className="flex items-center gap-2 rounded-lg border border-neutral-300 dark:border-neutral-700 px-6 py-3 text-sm hover:bg-black/5 dark:hover:bg-white/10">
      ...
      <section id="about" className="relative z-10 mx-auto max-w-6xl px-4 py-20">
        ...
        <div className="grid md:grid-cols-4 gap-4 text-sm">
          ...
          <div className="flex items-start gap-2"><Shuffle size={16}/> <span><b>Incubate</b> rapidly with lean prototyping and validation.</span></div>
          <div className="flex items-start gap-2"><Rocket size={16}/> <span><b>Launch</b> under the SV umbrella to prove traction.</span></div>
          <div className="flex items-start gap-2"><Building2 size={16}/> <span><b>Spin-Out</b> into independent companies.</span></div>
        </div>
      </section>
      ...
      <section id="portfolio" className="relative z-10 mx-auto max-w-6xl px-4 py-20">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {[
            { name: "Travel Guardian", desc: "AI-powered travel safety and booking platform.", url: "https://travelguardian.app" },
            { name: "HomeLog", desc: "Smart home maintenance & family coordination app.", url: "#" },
            { name: "Forzaen", desc: "EV-focused aftermarket accessories & 3D design studio.", url: "#" },
            { name: "FlexEnd", desc: "Weekend productivity app for flexible lifestyles.", url: "#" },
          ].map((v) => (
            <div key={v.name} className="rounded-2xl border border-white/10 bg-white/5 p-5 hover:bg-white/10 dark:bg-white/5 transition">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-semibold">{v.name}</h3>
                  <p className="text-sm text-neutral-500 dark:text-neutral-400">{v.desc}</p>
                </div>
                <a href={v.url} target="_blank" rel="noreferrer" className="rounded-xl border px-3 py-1 text-xs hover:bg-white/10 flex items-center gap-1">
      ...
      <section id="contact" className="relative z-10 mx-auto max-w-6xl px-4 py-20">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <p className="text-neutral-600 dark:text-neutral-300 mb-4">For partnerships, press, or vendor inquiries:</p>
            <a href="mailto:info@suszynskiventures.com" className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 hover:bg-black/5 dark:hover:bg-white/5">
              <Mail size={16}/> info@suszynskiventures.com
            </a>
            <div className="mt-4">
              <a href="https://www.linkedin.com" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm hover:text-white">
                <Linkedin size={16}/> LinkedIn
              </a>
            </div>
          </div>
          <form className="space-y-4">
            <input className="w-full rounded-xl border border-white/10 bg-black/5 dark:bg-black/30 p-2 text-sm" placeholder="Your name" />
            <input type="email" className="w-full rounded-xl border border-white/10 bg-black/5 dark:bg-black/30 p-2 text-sm" placeholder="you@company.com" />
            <textarea rows="4" className="w-full rounded-xl border border-white/10 bg-black/5 dark:bg-black/30 p-2 text-sm" placeholder="Your message"></textarea>
            <button type="button" className="inline-flex items-center gap-2 rounded-2xl bg-black/5 px-4 py-2 text-sm ring-1 ring-black/10 hover:bg-black/10 dark:bg-white/10 dark:ring-white/20 dark:hover:bg-white/15">
